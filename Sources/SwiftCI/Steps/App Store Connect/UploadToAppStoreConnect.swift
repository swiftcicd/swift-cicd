...
public struct UploadToAppStoreConnect: Step {
    ...
    public func run() async throws -> Output {
        ...
        project: if bundleShortVersion == nil {
            guard let project = xcodeProject ?? context.xcodeProject else {
                logger.debug("Couldn't detect bundle short version because xcode project wasn't specified explicitly or contextually.")
                break project
            }

            if let projectBundleShortVersion = try? getBuildSetting("MARKETING_VERSION", fromXcodeProject: project, quiet: true) {
