    ...
    public func run() async throws {
        ...
        if !shouldCreateKeychain {
            guard let certificateData = context.fileManager.contents(atPath: certificate) else {
                throw StepError("Failed to open certificate \(certificate)")
            }
        ...
        guard try keychainExists(keychain, in: verifyKeychains) else {
            throw StepError("Keychain setup failed \(keychain)")
        }
...
public extension Step where Self == InstallCertificate {
    ...
    static func installCertificate(
        ...
        let keychain = context.temporaryDirectory + "temp.keychain"
        let password = String.random(length: 20)
        return InstallCertificate(shouldCreateKeychain: true, keychain: keychain, keychainPassword: password, certificate: certificate, certificatePassword: certificatePassword)
    }
}
